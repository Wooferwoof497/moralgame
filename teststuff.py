...
main_hub_bg = pygame.transform.scale(pygame.image.load(os.path.join('pythonstuff','assets', 'mainhubtemp.png')),(WIDTH, HEIGHT))
tutorial_bg = pygame.transform.scale(pygame.image.load(os.path.join('pythonstuff','assets', 'tutorial.png')),(WIDTH, HEIGHT))
...
class MainHubRoom(Room):
    def __init__(self, background_image):
        ...
        self.player = pygame.Rect(100, 300, 16, 41)
    ...
    def handle_events(self, events): 
        for event in events:
            if event.type == pygame.QUIT:
                ...
                print("key pressed!")
                    #if self.player.colliderect(self.exit_n):
...
def player_movement(keys_pressed, player):
    ...
    if keys_pressed[pygame.K_d]: #right
        player.x += VEL
    #if keys_pressed[pygame.K_SPACE]: #shoot
          #  player.x -= VEL
#def drawtext(text):
